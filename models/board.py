...
class Board:
    ...
    # 隣の石に移動してget_stoneに返す
    def get_next_stone(self, row: int, col: int, dr: int, dc: int) -> int | None:
        return self.get_stone(row + dr, col + dc)
    
    def has_opponent_in_direction(self, row: int, col: int, 
                                  dr: int, dc:int, stone:int) -> bool: # 相手の石を挟めるか
        row += dr
        col += dc
        if not self.is_inside(row, col):
            return False
    ...
    def flip_in_direction(self, row: int, col:int, 
                          dr: int, dc:int, stone:int) -> None: # ひっくり返す
